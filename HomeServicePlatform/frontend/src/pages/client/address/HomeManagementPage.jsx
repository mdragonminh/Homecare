...
import { Plus, Search, X, Eye, Pencil, Trash2, Home, Filter, MapPin, User, Building, ChevronLeft, ChevronRight } from "lucide-react";
...
const homeTypeColors = {
  ...
  apartment: { bg: "bg-teal-100", text: "text-teal-700", icon: "text-teal-600" },
  house: { bg: "bg-purple-100", text: "text-purple-700", icon: "text-purple-600" },
  villa: { bg: "bg-orange-100", text: "text-orange-700", icon: "text-orange-600" },
  condo: { bg: "bg-fuchsia-100", text: "text-fuchsia-700", icon: "text-fuchsia-600" }
};
...
export default function HomeManagementPage() {
  ...
  // ✅ Cập nhật fetchHomes để nhận các tham số phân trang, tìm kiếm, lọc
  const fetchHomes = async (page = 1, search = searchTerm, type = filterType) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        // ✅ Cập nhật state phân trang từ dữ liệu backend
  ...
  // ✅ Gọi fetchHomes ban đầu và khi các bộ lọc thay đổi
  useEffect(() => {
    fetchHomes(currentPage);
  }, [currentPage]);
  
  // ✅ Thêm useEffect để xử lý tìm kiếm/lọc
  useEffect(() => {
    const handler = setTimeout(() => {
      fetchHomes(1, searchTerm, filterType);
    }, 500); // Debounce
    return () => clearTimeout(handler);
  }, [searchTerm, filterType]);

  const handleDelete = () => {
    // In a real application, you would call an API to delete the home
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-gray-100">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          <div className={`bg-white rounded-2xl p-4 shadow-sm border border-gray-100 ${homeTypeColors.all.bg}`}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-gray-600">Tổng số nhà</p>
          ...
          {homeTypes.map((type) => {
            ...
            return (
              <div key={type} className={`bg-white rounded-2xl p-4 shadow-sm border border-gray-100 ${colors.bg}`}>
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-600">{homeTypeLabels[type]}</p>
                    <p className="text-2xl font-bold text-gray-900">{count}</p>
                  </div>
                  <div className="text-3xl">{homeTypeIcons[type]}</div>
                </div>
              </div>
            );
          })}
        ...
        {homes.length === 0 ? (
          <div className="text-center py-16">
            <div className="text-6xl mb-4">🏠</div>
            <h3 className="text-xl font-semibold text-gray-800 mb-2">Không tìm thấy bất động sản nào</h3>
                ...
                <div className="p-6 pb-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <span className="text-2xl">{homeTypeIcons[home.type]}</span>
                      <span className={`px-3 py-1 ${homeTypeColors[home.type].bg} ${homeTypeColors[home.type].text} text-xs font-medium rounded-full`}>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-gray-600">
                      ...
                      <span className="text-sm line-clamp-1">{home.address}</span>
                    </div>
                    
                    <div className="flex items-center gap-2 text-gray-600">
                      <User size={16} className="text-gray-400 flex-shrink-0" />
                      <span className="text-sm font-medium">{home.ownerName}</span>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg transform transition-all">
            <div className="p-6 border-b border-gray-100">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <span className="text-3xl">{homeTypeIcons[selectedHome.type]}</span>
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">{selectedHome.name}</h2>
                    <span className={`px-3 py-1 ${homeTypeColors[selectedHome.type].bg} ${homeTypeColors[selectedHome.type].text} text-xs font-medium rounded-full`}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-gray-700">Chủ sở hữu</p>
