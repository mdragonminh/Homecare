...
namespace HSP.Service.Test.Implementations
{
	...
	{
		private readonly Mock<IRepository<HSP.Core.Entities.File, Guid>> _mockFileRepo;
		...
		{
			_mockFileRepo = new Mock<IRepository<HSP.Core.Entities.File, Guid>>();
		...
		public async Task UploadAsync_ValidInput_ShouldSaveFileAndReturnDto()
		{
			...
			_mockFileRepo.Verify(r => r.AddAsync(It.IsAny<HSP.Core.Entities.File>()), Times.Once);
		...
		{
			var file = new HSP.Core.Entities.File { Id = Guid.NewGuid(), FilePath = "uploads/test.jpg" };
			_mockFileRepo.Setup(r => r.GetAll())
					.Returns(new List<HSP.Core.Entities.File> { file }.BuildMock());
			...
			_mockFileRepo.Verify(r => r.HardDelete(It.IsAny<HSP.Core.Entities.File>()), Times.Once);
			_mockUow.Verify(u => u.SaveChangesAsync(), Times.Once);
		}
		[Fact]
		...
		public async Task GetFilesAsync_ShouldReturnFilesForObject()
		{
			...
			var objectType = new ObjectType { Id = Guid.NewGuid(), Name = "technician" };
			var file = new HSP.Core.Entities.File { Id = Guid.NewGuid(), FileName = "file1.jpg" };
