...
import {
  Loader2, PlusCircle, Edit, MapPin, Search, Home, ChevronsDown, XCircle, CheckCircle,
  AlertTriangle, Star, Globe, Check, ChevronDown, Wrench, Locate,
} from "lucide-react";
...
export function FindTechnicianPage({ loggedInUser }) {
  ...
  const renderStatusMessage = (message) => {
    ...
    if (message.startsWith(t("success.home_added").substring(0, 10)) || message.startsWith(t("success.home_edited").substring(0, 10)) || message.startsWith(successKey) || message.startsWith(t("success.geocode_success").substring(0, 10))) {
      icon = <CheckCircle className="h-5 w-5 mr-2 flex-shrink-0" />; 
      colorClass = "text-green-800 bg-green-100 border-green-300"; 
    } else if (message.startsWith(errorKey) || message.startsWith(t("error.address_invalid").substring(0, 10)) || message.startsWith(t("error.geocode_failed").substring(0, 10))) {
      icon = <AlertTriangle className="h-5 w-5 mr-2 flex-shrink-0" />; 
      colorClass = "text-red-700 bg-red-100 border-red-300"; 
    } else if (message.startsWith(searchingKey) || message.startsWith(t("ui.searching_technicians").substring(0, 10)) || message.startsWith(t("ui.loading_data").substring(0, 10)) || message.startsWith(t("ui.getting_current_location").substring(0, 10))) {
  ...
  return (
    <div className="p-4 md:p-8 max-w-full lg:max-w-7xl xl:max-w-full mx-auto pt-10 min-h-screen bg-white"> {/* Bỏ nền gradient */}
      {/* Modals */}
      ...
      <h1 className="text-3xl md:text-4xl font-extrabold mb-8 text-gray-900 border-b-4 border-blue-500 pb-4"> {/* Tăng kích thước tiêu đề */}
          ...
          <div className="p-6 border border-gray-200 rounded-2xl shadow-xl bg-white space-y-5 flex-1 flex flex-col"> 
            ...
            {loggedInUser && homes.length > 0 ? (
              <div className="space-y-3">
                <div>
                  <label className="block text-base font-semibold text-gray-800 mb-1"> {/* Tăng text size */}
                    {t("ui.select_service_address")}
                  </label>
                  <div className="flex flex-wrap gap-2 items-stretch"> {/* Tăng gap */}
                    <select
                      value={selectedHomeId || ""}
                      onChange={handleAddressSelection}
                      className="flex-grow min-w-[200px] h-11 px-4 border-2 border-gray-300 rounded-xl bg-white text-gray-900 hover:border-blue-500 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all cursor-pointer shadow-sm font-medium text-base" // Tăng height, padding, text size, bo góc
                    >
                    ...
                    {currentHomeData && (
                      <button
                        ...
                        className="w-11 h-11 bg-gradient-to-br from-amber-400 to-amber-600 text-white rounded-xl shadow-md hover:shadow-lg hover:scale-[1.05] transition-all active:scale-95 flex items-center justify-center flex-shrink-0" // Tăng kích thước button
                    ...
                    <button
                      ...
                      className="w-11 h-11 bg-gradient-to-br from-green-500 to-green-700 text-white rounded-xl shadow-md hover:shadow-lg hover:scale-[1.05] transition-all active:scale-95 flex items-center justify-center flex-shrink-0" // Tăng kích thước button
                      title={t("ui.add_new")}
                    >
                  ...
                  {currentHomeData && (
                    <p className="mt-3 text-base text-blue-900 flex items-start gap-2 p-3 bg-gradient-to-r from-blue-50 to-cyan-50 border-2 border-blue-300 rounded-xl shadow-sm"> {/* Tăng padding, margin-top, text size */}
              ...
              <div className="text-center p-6 border-2 border-dashed border-gray-300 rounded-xl bg-gray-50"> {/* Tăng padding, bo góc */}
            ...
            <div className="relative">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-11 p-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-base font-medium pr-10 shadow-sm hover:border-gray-400" // Tăng height, padding, text size, bo góc
                ...
                <div className="mt-3 p-3 bg-gradient-to-r from-blue-50 to-blue-100 border-2 border-blue-300 text-blue-900 rounded-xl shadow-sm">
                  <div className="flex flex-wrap gap-2">
                    {selectedServiceNames.map((name, index) => (
                      <span key={index} className="flex items-center text-sm px-3 py-1.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-full shadow-md hover:shadow-lg transition-all">
                        {name}
                        <button
              ...
              {(isServiceDropdownOpen && !isServicesLoading) && (
                <ul className="absolute z-20 w-full bg-white border-2 border-gray-300 mt-1 rounded-xl shadow-2xl max-h-60 overflow-y-auto"> {/* Tăng bo góc, shadow */}
                  {filteredServices.map((service) => {
                    ...
                    return (
                      <li
                        ...
                        className={`p-3 cursor-pointer hover:bg-blue-50 flex items-center justify-between transition-colors text-base ${isSelected ? 'bg-blue-100 text-blue-700 font-semibold' : 'text-gray-800'}`}
                      >
                        {service.name}
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full h-11 p-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all text-base font-medium pr-14 shadow-sm hover:border-gray-400" // Tăng height, padding, text size, bo góc
            ...
            {!loggedInUser || (loggedInUser && !selectedHomeId) ? (
              <div className="relative space-y-2">
                ...
                <input
                  ...
                  className="w-full h-11 p-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm hover:border-gray-400 text-base" // Tăng height, padding, text size, bo góc
            ...
            <button
              ...
              // Bỏ điều kiện disabled cho coords.latitude và searchRadius để nút luôn nhấn được (chỉ disabled khi đang searching)
              disabled={isSearching} 
              className="w-full mt-5 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-bold text-base rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.01] transition-all disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center active:scale-[0.99]"
            >
              {isSearching ? (
                <>
            ...
            {loggedInUser && (
              <button
                ...
                className="w-full mt-3 py-3 bg-gradient-to-r from-red-600 to-red-700 text-white font-bold text-base rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.01] transition-all disabled:opacity-60 disabled:cursor-not-allowed disabled:hover:scale-100 flex items-center justify-center active:scale-[0.99]"
              >
                {isMatching ? (
                  <>
                    <Loader2 className="h-5 w-5 text-white mr-2 animate-spin" />
        ...
        <div className="lg:w-1/2 flex relative min-h-[450px]">
          ...
          {!loggedInUser || (loggedInUser && !selectedHomeId) ? (
            <button
              ...
              title={t("ui.use_my_current_location", { defaultValue: "Sử dụng Vị trí Hiện tại của tôi" })}
            >
              {isGettingLocation ? (
      ...
      <div className="p-6 border-2 border-gray-200 rounded-2xl bg-white shadow-xl max-h-[500px] overflow-y-auto mt-6"> {/* Tăng padding, bo góc, shadow và max height */}
        ...
        {technicians && technicians.length > 0 && (
          <ul className="space-y-4"> {/* Tăng space-y */}
            {technicians.map((tech, index) => (
              <li
                key={tech.id}
                className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 border-2 border-gray-200 rounded-xl hover:border-blue-500 hover:bg-blue-50 hover:shadow-lg transition-all duration-200 cursor-pointer" // Tăng padding, bo góc, hover effect
              >
                <div className="flex items-center mb-3 sm:mb-0 w-full sm:w-auto">
                  <span
                    className={`text-2xl font-extrabold mr-3 w-8 text-center ${ // Tăng kích thước số thứ tự
                      index < 3 ? "text-red-600" : "text-gray-500"
                    }`}
                  >
                    {index + 1}
                  </span>
                  <div className="flex-shrink-0 w-12 h-12 bg-gradient-to-br from-red-100 to-red-50 rounded-full flex items-center justify-center mr-3 border-2 border-red-400 shadow-md"> {/* Tăng kích thước avatar */}
                  ...
                  <div className="flex-grow">
                    ...
                    <div className="flex items-center text-base text-gray-600 mt-1.5 gap-2"> {/* Tăng text size, gap */}
                ...
                <button className="text-base px-6 py-2 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-xl hover:shadow-xl hover:scale-105 transition-all active:scale-95 shadow-lg w-full sm:w-auto mt-2 sm:mt-0"> {/* Tăng text size, padding, bo góc, shadow */}
