...
const mapServiceTitleToIcon = (title) => {
  ...
  if (lowerTitle.includes("cắt tỉa") || lowerTitle.includes("cây"))
    return Scissors;
  return Wrench;
};
...
export function HomePage({ onShowRegister, loggedInUser }) {
  ...
  // Index bắt đầu của danh sách 6 dịch vụ đang hiển thị
  ...
  // Logic tự động nhảy 1 dịch vụ sau mỗi 5s
  useEffect(() => {
  ...
  // Lấy ra đúng 6 dịch vụ dựa trên startIndex (xoay vòng)
  const visibleServices = useMemo(() => {
    if (services.length === 0) return [];
    let items = [];
    for (let i = 0; i < 6; i++) {
      items.push(services[(startIndex + i) % services.length]);
    }
    return items;
  }, [services, startIndex]);
    ...
    <div className="bg-white min-h-screen">
          ...
          <div className="container mx-auto px-6">
            ...
            >
              ...
              <p className="text-base md:text-lg mt-4 text-gray-200">
                Kết nối nhanh với thợ uy tín • Báo giá tức thì • Thanh toán an
                toàn
              </p>
              <div className="flex flex-col sm:flex-row gap-4 mt-8">
                <button
                  onClick={onShowRegister}
                  className="bg-white text-blue-600 font-semibold px-7 py-3.5 rounded-xl shadow-lg flex items-center justify-center gap-2"
                >
                  Bắt đầu ngay <ArrowRight className="w-5 h-5" />
                </button>
                <a
                  href="https://drive.google.com/file/d/1lKYEkP4KolPtMShne-RF4uq6ptgVOlUY/view"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="border border-white/70 text-white px-7 py-3.5 rounded-xl backdrop-blur-sm flex items-center justify-center gap-2 hover:bg-white/10 transition"
                >
      ...
      <section className="py-20">
        <div className="container mx-auto px-6">
          ...
          {/* Container bao quanh dùng relative để định vị nút mũi tên */}
          <div className="relative group">
            {loadingServices ? (
              ...
              <>
                {/* Grid dịch vụ: Thêm pr-16 để tránh nội dung bị nút mũi tên che mất */}
                    ...
                    {visibleServices.map((s, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <h3 className="text-center text-sm font-semibold text-gray-800 line-clamp-1">
                            {s.name}
                          </h3>
                          <p className="text-center text-xs text-gray-500 mt-2 line-clamp-2">
                            {s.description || "Dịch vụ chất lượng cao"}
                          </p>
                        </motion.div>
                      );
                    })}
                ...
                {/* NÚT MŨI TÊN: Căn giữa dọc tuyệt đối bằng top-1/2 và -translate-y-1/2 */}
                <button
                  onClick={handleNextService}
                  className="absolute -right-4 md:-right-6 top-1/2 -translate-y-1/2 z-20 p-2 text-blue-600 hover:text-blue-800 hover:scale-110 transition-all"
          ...
          {loadingTechnicians ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
              {[...Array(8)].map((_, i) => (
                <div
                  key={i}
                  className="bg-gray-200 rounded-2xl h-64 animate-pulse shadow"
                />
              ))}
            </div>
          ) : (
