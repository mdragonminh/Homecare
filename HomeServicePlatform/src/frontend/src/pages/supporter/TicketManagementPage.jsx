...
import {
  ...
  Zap,
  XCircle,
  X, // Icon để đóng modal
  Send, // Icon Gửi
} from "lucide-react";
import { getMyTickets, assignTechnician, updateTicketStatus } from "../../services/ticketApi";
// --- SỬA IMPORT ---
import { technicianApi } from "../../services/technicianApi"; 
import { toast } from "sonner"; // Dùng để thông báo

// --- Định nghĩa Status (khớp với Backend Enum) ---
// Key là string trả về từ API (ToString()), value là int để gửi đi
const statusMap = {
  "NotAccepted": 0,
  "Pending": 1,
  "InProgress": 2,
...
// Dùng để hiển thị trong dropdown
const statusOptions = [
...
const TicketManagementPage = () => {
  ...
  // --- State cho Modals ---
  const [isSubmitting, setIsSubmitting] = useState(false); // Chống click nhiều lần
  
  // State cho Modal "Cập nhật Trạng thái"
  const [isStatusModalOpen, setIsStatusModalOpen] = useState(false);
  const [selectedTicket, setSelectedTicket] = useState(null);
  const [newStatus, setNewStatus] = useState(0); // Sẽ lưu giá trị int (0, 1, 2, 3)

  // State cho Modal "Gán Tech"
  const [isAssignModalOpen, setIsAssignModalOpen] = useState(false);
  const [technicianList, setTechnicianList] = useState([]);
  const [selectedTechnicianId, setSelectedTechnicianId] = useState("");
  // -------------------------

  // Tách hàm fetchTickets ra để có thể gọi lại sau khi submit
  const fetchTickets = useCallback(async () => {
    ...
    if (response.success) {
      ...
      setTickets([]);
      toast.error(response.message || "Lỗi khi tải tickets."); // Hiển thị lỗi
    }
    setLoading(false);
  }, [pagination.pageNumber, pagination.pageSize]);

  // useEffect để gọi fetchTickets
  useEffect(() => {
    fetchTickets();
  }, [fetchTickets]); // Dependency là hàm useCallback
  ...
  // --- Hàm xử lý Modal Cập nhật Trạng thái ---
  const openStatusModal = (ticket) => {
    setSelectedTicket(ticket);
    // Lấy giá trị int của status hiện tại
  ...
  const handleStatusSubmit = async (e) => {
    ...
    const response = await updateTicketStatus({
      ...
      newStatus: parseInt(newStatus) // Gửi đi giá trị int
    });

    if (response.success) {
      toast.success("Cập nhật trạng thái thành công!");
      closeStatusModal();
      await fetchTickets(); // Tải lại danh sách tickets
    } else {
  ...
  // --- Hàm xử lý Modal Gán Tech ---
  const openAssignModal = async (ticket) => {
    setSelectedTicket(ticket);
    setIsAssignModalOpen(true);
    
    // --- SỬA ĐOẠN NÀY ---
    // Giả sử DTO của KTV trả về có 'id' (là TechnicianProfileId)
    // Và 'technicianId' trong ticket cũng là TechnicianProfileId
    setSelectedTechnicianId(ticket.technicianId || ""); 

    // Tải danh sách tech
    // Gọi hàm 'getTechnicians' từ file 'technicianApi' của bạn
    // Giả sử ApprovalStatus: 1 là "Approved"
    const res = await technicianApi.getTechnicians({ ApprovalStatus: 1, PageSize: 500, PageNumber: 1 });
    
    if (res.success) {
      // API của bạn trả về PagedList, nên data nằm trong .items
  ...
  const handleAssignSubmit = async (e) => {
    ...
    setIsSubmitting(true);
    // 'selectedTechnicianId' giờ là 'tech.id' (TechnicianProfile.Id)
    // Hàm 'assignTechnician' của bạn cũng mong đợi TechnicianProfile.Id
    ...
    if (response.success) {
        ...
        closeAssignModal();
        await fetchTickets(); // Tải lại danh sách
    } else {
                ...
                <div>
                  ...
                  {technicianList.length > 0 ? (
                    ...
                    >
                      ...
                      {/* --- SỬA LOGIC MAPPING --- */}
                      {/* Giả định DTO trả về (TechnicianProfileResponseDto) có:
                        - id (là TechnicianProfile.Id)
                        - fullName (là AppUser.FullName)
                      */}
