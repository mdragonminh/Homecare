...
export function AuthButtons({
  ...
  onLogout,
  navigate,
  isMobile = false, // <-- Giá trị mặc định
}) {
  ...
  useEffect(() => {
    if (isMobile) return; // Bỏ qua logic click outside cho Mobile Menu
  ...
  const handleLogoutClick = () => {
    onLogout();
    // Đóng menu sau khi logout, áp dụng cho cả Mobile và Desktop
    setIsDropdownOpen(false);
  };

  const handleNavigate = (path) => {
    navigate(path);
    // Đóng menu sau khi navigate, áp dụng cho cả Mobile và Desktop
    setIsDropdownOpen(false);
  };
  
  // Class cho các nút trong menu mobile đã đăng nhập (đã chỉnh lại)
  const mobileMenuItemClass = "flex items-center w-full px-5 py-3 text-sm text-gray-700 rounded-xl hover:bg-gray-50 hover:text-blue-600 transition-all duration-200 group";
  const mobileLogoutItemClass = "flex items-center w-full px-5 py-3 text-sm text-red-600 rounded-xl hover:bg-red-50 transition-all duration-200 group";


  // ======================================================================
  // ===== RENDER CHƯA ĐĂNG NHẬP (Guest User) - Mobile và Desktop tách biệt  =====
  // ======================================================================
  if (!loggedInUser) {
    if (isMobile) {
      // Mobile Guest View: Hiển thị 3 nút dọc, không cần dropdown toggle
      return (
        <div className="w-full space-y-3">
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="flex items-center w-full px-3 py-3 text-sm text-gray-700 rounded-xl hover:bg-gray-50 hover:text-green-600 transition-all duration-200 group border border-gray-200 hover:border-green-200"
            >
            ...
            <button
              ...
              className="flex items-center w-full px-3 py-3 text-sm text-gray-700 rounded-xl hover:bg-orange-50 hover:text-orange-600 transition-all duration-200 group border border-dashed border-gray-300 hover:border-orange-300"
            >
  ...
  // ======================================================================
  // ===== RENDER CHÍNH: LOGGED IN USER VÀ GUEST DESKTOP (Sử dụng Toggle) =====
  // ======================================================================
  
  // Xác định class cho Menu Container
  const menuContainerClasses = isMobile
    ? `w-full mt-2 space-y-1 ${!isDropdownOpen ? 'hidden' : 'block'}` // Mobile: ẩn/hiện và layout dọc
    : "absolute right-0 mt-2 w-64 origin-top-right rounded-xl bg-white shadow-2xl ring-1 ring-black/5 border border-gray-100 transform transition-all duration-300 ease-out scale-95 animate-in zoom-in-95 p-2"; // Desktop: dropdown
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={
          isMobile && loggedInUser
            ? "group flex items-center justify-between w-full px-4 py-3 text-base font-semibold text-gray-800 bg-gray-50 rounded-xl hover:bg-gray-100 transition-all duration-200 border border-gray-200"
            : "group flex items-center space-x-2 px-3 py-2 text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-blue-700 rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-300 shadow-md hover:shadow-blue-500/25 transform hover:scale-105"
        }
      >
        {loggedInUser ? (
          <>
            <div className="flex items-center space-x-2">
                <div className={`w-7 h-7 ${isMobile ? 'bg-blue-100' : 'bg-white/20'} rounded-full flex items-center justify-center`}>
                    <UserCircle className={`w-4 h-4 ${isMobile ? 'text-blue-600' : 'text-white'}`} />
                </div>
                <span className={`${isMobile ? 'text-gray-900' : 'hidden sm:inline text-white'} font-semibold`}>
                    {loggedInUser.name || t("ui.account")}
                </span>
            </div>
            {/* Chevron (luôn hiện, quay khi mở) */}
            <ChevronDown
      ...
      {isDropdownOpen && (
        <div className={menuContainerClasses}>
          {loggedInUser ? (
            // Logged In User Menu (Mobile/Desktop)
            <>
              {/* User Info Header (Chỉ hiện khi là Desktop, hoặc là Mobile và đang mở menu con) */}
              <div className={`px-3 py-2 bg-gradient-to-r from-blue-50 to-indigo-50 ${isMobile ? 'rounded-lg' : 'rounded-lg mb-2'} border border-blue-100`}>
                <div className="flex items-center space-x-2">
              ...
              <div className="space-y-1">
                ...
                {loggedInUser.role === "admin" && (
                  <button
                    ...
                    className={isMobile ? mobileMenuItemClass : "flex items-center w-full px-3 py-2.5 text-sm text-gray-700 rounded-xl hover:bg-gray-50 hover:text-blue-600 transition-all duration-200 group"}
                  >
                ...
                {loggedInUser.role === "operator" && (
                  <button
                    ...
                    className={isMobile ? mobileMenuItemClass.replace('blue', 'green') : "flex items-center w-full px-3 py-2.5 text-sm text-gray-700 rounded-xl hover:bg-gray-50 hover:text-green-600 transition-all duration-200 group"}
                  >
                    <div className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center mr-2 group-hover:bg-green-100 transition-colors">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
                ...
                {loggedInUser.role === "equipmentmanager" && (
                  <button
                    ...
                    className={isMobile ? mobileMenuItemClass.replace('blue', 'orange') : "flex items-center w-full px-3 py-2.5 text-sm text-gray-700 rounded-xl hover:bg-gray-50 hover:text-orange-600 transition-all duration-200 group"}
                  >
                ...
                {loggedInUser.role === "technician" && (
                  <button
                    ...
                    className={isMobile ? mobileMenuItemClass.replace('blue', 'purple') : "flex items-center w-full px-3 py-2.5 text-sm text-gray-700 rounded-xl hover:bg-gray-50 hover:text-purple-600 transition-all duration-200 group"}
                  >
                ...
                {loggedInUser.role === "supporter" && (
                  <button
                    ...
                    className={isMobile ? mobileMenuItemClass : "flex items-center w-full px-3 py-2.5 text-sm text-gray-700 rounded-xl hover:bg-blue-50 hover:text-blue-600 transition-all duration-200 group"}
                  >
                ...
                <button
                  ...
                  className={isMobile ? mobileMenuItemClass.replace('blue', 'purple') : "flex items-center w-full px-3 py-2 text-sm text-gray-700 rounded-lg hover:bg-purple-50 hover:text-purple-600 transition-all duration-200 group"}
                >
                ...
                {loggedInUser.role !== "technician" && loggedInUser.role !== "supporter" && loggedInUser.role !== "equipmentmanager" && (
                  <button
                    ...
                    className={isMobile ? mobileMenuItemClass.replace('blue', 'green') : "flex items-center w-full px-3 py-2 text-sm text-gray-700 rounded-lg hover:bg-green-50 hover:text-green-600 transition-all duration-200 group"}
                  >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <div className="text-xs text-green-500">{t("ui.add_new_delivery_address")}</div>
                    </div>
                  </button>
                )}
                ...
                <button
                  ...
                  className={isMobile ? mobileLogoutItemClass : "flex items-center w-full px-3 py-2 text-sm text-red-600 rounded-lg hover:bg-red-50 transition-all duration-200 group"}
                >
            ...
            <>
              ...
              <div className="space-y-1">
                {/* LOGIN */}
                <button
                  onClick={() => { onShowLogin(); setIsDropdownOpen(false); }}
                ...
                {/* REGISTER CUSTOMER */}
                <button
                  onClick={() => { onShowRegister(); setIsDropdownOpen(false); }}
