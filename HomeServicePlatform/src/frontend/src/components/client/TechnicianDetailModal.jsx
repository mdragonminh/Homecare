...
import { Modal, Tag, Rate, Spin, Empty, Avatar, Card, Divider } from "antd";
import {
  UserOutlined,
  PhoneOutlined,
  MailOutlined,
...
export default function TechnicianDetailModal({ visible, onClose, technicianId }) {
  ...
  const loadTechnicianFeedbacks = async () => {
    ...
    try {
      // Get all bookings for this technician with feedback
      const response = await bookingApi.getAllBookingsForTechnician(technicianId);
      if (response.success && response.data?.items) {
        // Extract feedbacks from bookings
        const feedbackList = response.data.items
          ...
          .map(booking => ({
            ...
            customerName: booking.customer?.fullName || booking.customer?.email || "Anonymous",
            createdAt: booking.feedback.createdAt || booking.completedAt,
          }));
        setFeedbacks(feedbackList);
      }
    } catch (error) {
      console.error("Error loading technician feedbacks:", error);
    } finally {
  ...
  return (
    <Modal
      title={
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span>{t("ui.technician_details", { defaultValue: "Chi tiết kỹ thuật viên" })}</span>
        </div>
      }
      open={visible}
      onCancel={onClose}
    ...
    >
      {loading ? (
        ...
        <div style={{ maxHeight: "70vh", overflowY: "auto" }}>
          ...
          <Card style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <Avatar size={80} icon={<ToolOutlined />} style={{ backgroundColor: "#1890ff" }}>
                {technicianDetail.fullName?.charAt(0)}
              </Avatar>
              <div style={{ flex: 1 }}>
                <h3 style={{ margin: 0, fontSize: 20 }}>{technicianDetail.fullName}</h3>
              ...
              <div style={{ textAlign: "center" }}>
                ...
                <div style={{ color: "#8c8c8c", fontSize: 12 }}>
                  {feedbacks.length} {t("ui.reviews", { defaultValue: "đánh giá" })}
                </div>
              </div>
            </div>
          </Card>
          ...
          <Card style={{ marginBottom: 16 }}>
            <div style={{ marginBottom: 16 }}>
              <strong>
                <TrophyOutlined /> {t("ui.experience", { defaultValue: "Kinh nghiệm" })}:
              </strong>{" "}
              {technicianDetail.experienceYears} {t("ui.years", { defaultValue: "năm" })}
            </div>

            {technicianDetail.services && technicianDetail.services.length > 0 && (
              <div>
                <strong style={{ display: "block", marginBottom: 8 }}>
                  <ToolOutlined /> {t("ui.skills", { defaultValue: "Kỹ năng" })}:
                </strong>
          ...
          <Card
            title={
              <span>
                <StarOutlined /> {t("ui.ratings_reviews", { defaultValue: "Đánh giá & Nhận xét" })}
              </span>
            }
          ...
          >
            {loadingFeedbacks ? (
                      ...
                      {Object.entries(getRatingDistribution())
                        ...
                        .map(([star, count]) => (
                          <div key={star} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                            ...
                            <span style={{ width: 30, fontSize: 12, color: "#8c8c8c" }}>{count}</span>
                          </div>
                        ))}
                    </div>
                  </div>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ fontSize: 12, color: "#8c8c8c" }}>
                            {feedback.createdAt ? dayjs(feedback.createdAt).format("DD/MM/YYYY HH:mm") : ""}
                          </div>
                        </div>
                        <Rate disabled value={feedback.rating} style={{ fontSize: 14 }} />
                      </div>
                      {feedback.comment && (
                        <div style={{ paddingLeft: 40, color: "#595959" }}>{feedback.comment}</div>
                      )}
                    </div>
                  ))}
                </div>
              ...
              <Empty
                description={t("ui.no_ratings_yet", { defaultValue: "Chưa có đánh giá" })}
          ...
          {/* Certificates */}
          {technicianDetail.certificateFiles && technicianDetail.certificateFiles.length > 0 && (
            <Card
              title={
                <span>
                  <FileTextOutlined /> {t("ui.certificates", { defaultValue: "Chứng chỉ" })}
                </span>
              }
            >
                ...
                {technicianDetail.certificateFiles.map((file, index) => (
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: "#8c8c8c" }}>
                        {t("ui.certificate", { defaultValue: "Chứng chỉ" })} #{index + 1}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        <Empty description={t("ui.no_data", { defaultValue: "Không có dữ liệu" })} />
      )}
    </Modal>
  );
}
