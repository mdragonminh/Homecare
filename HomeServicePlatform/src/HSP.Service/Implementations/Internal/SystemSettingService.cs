...
namespace HSP.Service.Implementations.Internal
{
    public class SystemSettingService : BaseService, ISystemSettingService
    {
        ...
        public async Task<Guid> CreateSystemSettingAsync(CreateSystemSettingDto input)
        {
            if (input == null)
            {
                throw new ArgumentNullException(_localizer["InputCannotBeNull"]);
            }
            var existing = await _systemSettingRepository.GetAll()
                .AnyAsync(s => s.Key == input.Key);
            if (existing)
            {
                throw new InvalidOperationException(_localizer["SettingKeyAlreadyExists"]);
            }
            if (SystemSettingRegistry.All.TryGetValue(input.Key, out var def))
            {
                input.Group ??= def.Group;
        ...
        {
            string? value = await GetSettingValueAsync(key); 

            if (string.IsNullOrWhiteSpace(value))
            {
                if (SystemSettingRegistry.All.TryGetValue(key, out var def))
            ...
            {
                if (SystemSettingRegistry.All.TryGetValue(key, out var def))
                    return (T)Convert.ChangeType(def.DefaultValue, typeof(T));
